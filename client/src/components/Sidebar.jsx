...
const Sidebar = ({ isOpen = false, onClose = () => {}, unreadCount = 0 }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[17rem] flex-col bg-[#0c1729] px-3 text-white transition-transform duration-200 lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto custom-scrollbar pr-1.5">
          {items.map((item) => {
            ...
            return (
              <NavLink
                ...
                onClick={onClose}
                className={({ isActive }) => `flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? "bg-primary text-white shadow-sm" : "text-slate-300 hover:bg-white/[0.07] hover:text-white"}`}
              >
                ...
                {isNotifications && unreadCount > 0 && (
                  <span className="grid h-5 min-w-[20px] place-items-center rounded-full bg-red-500 px-1.5 text-[10px] font-black text-white">
                    {unreadCount}
                  </span>
                )}
              </NavLink>
        ...
        <div className="mb-4 mt-2 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 truncate text-xs capitalize text-slate-400">{user?.role === "admin" ? "Grid Admin" : (user?.role || "member")}</p>
            </div>
          </div>
