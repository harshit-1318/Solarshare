...
export let isSimulationRunning = true;
let intervalId = null;

export const setSimulationState = (state) => {
  isSimulationRunning = Boolean(state);
  console.log(`Smart meter simulator state set to: ${isSimulationRunning ? "RUNNING" : "STOPPED"}`);
};

export const runSimulationTick = async () => {
  if (!isSimulationRunning) {
    console.log("Smart meter simulation tick skipped (disabled)");
    return;
  }
